...
def delete_old_file(save_folder, date_time_now):
    excel_list = glob(os.path.join(save_folder, "*"))
    excel_list = sorted(excel_list)
    for excel_file in excel_list:
        if os.path.splitext(excel_file)[-1] != ".xls":
            continue
        excel_time = excel_file.split("/")[-1].split(".xls")[0].split("_")[-1]
        date_time_file = datetime.datetime.strptime(excel_time, "%Y%m%d%H%M%S")
        ## time diff
        time_diff = (date_time_now - date_time_file).days
        if time_diff >= 1:
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        save_path = os.path.join(save_folder, "data_weather_{}.xls".format(date_time_str))
        work(url, headers, save_path)
        delete_old_file(save_folder, date_time_now)
        time.sleep(1*10)
